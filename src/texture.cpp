...
bool Texture::loadFromFile(std::string path, SDL_Window* window, SDL_Renderer* renderer)
{
	// load pixels
	...
	else
	{
		...
		{
			std::cerr << "Failed to texture from pixels from " << path.c_str() << std::endl;
		}
	}
...
bool Texture::loadPixelsFromFile(std::string path, SDL_Window* window)
{
	...
	if(!loadedSurface)
	{
		std::cerr << "Unable to load image " << path.c_str() << "! SDL_image Error: " << SDL_GetError() << std::endl;
	}
	else
	{
		...
		if(!mSurfacePixels)
		{
			std::cerr << "Unable to convert loaded surface to display format! SDL Error: " << SDL_GetError() << std::endl;
		}
		else
		{
...
bool Texture::loadFromPixels(SDL_Renderer* renderer)
{
	...
	else
	{
		...
		if( mTexture == NULL )
		{
			std::cerr << "Unable to create texture from loaded pixels! SDL Error: " << SDL_GetError() << std::endl;
		}
		else
		{
			//Get image dimensions
